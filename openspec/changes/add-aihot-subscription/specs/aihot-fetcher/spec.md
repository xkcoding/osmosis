...
fetcher MUST 在返回的 `FetchResult` 中填充 `notifyBody` 字段，内容为「日报段」的 markdown（lead + sections + flashes），不包含精选池段。`notifyBody` MUST NOT 超过 4 KB；超出时尾部截断并以 `…\n（已截断）` 结尾。

#### Scenario: 含 lead + sections 的标准日

- **WHEN** 日报含 lead 与至少一个非空 section
- **THEN** `result.notifyBody` 字符串非空且不含「精选池」标题

#### Scenario: 超长截断

- **WHEN** 渲染后的 notifyBody markdown 超过 4096 字节
- **THEN** 实际字段被截断到 ≤ 4096 字节，且以 `…\n（已截断）` 结尾
